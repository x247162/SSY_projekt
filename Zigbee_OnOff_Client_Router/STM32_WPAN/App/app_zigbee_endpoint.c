...
#define APP_ZIGBEE_ENDPOINT               16u
#define APP_ZIGBEE_ENDPOINT2              15u
...
#define OnOffClient1                      pstZbCluster[0]
#define OnOffClient2                      pstZbCluster[1]
...
void APP_ZIGBEE_ConfigEndpoints(void)
{
  ...
  /* Add OnOff Client Cluster for Endpoint 1 */
  stZigbeeAppInfo.OnOffClient1 = ZbZclOnOffClientAlloc( stZigbeeAppInfo.pstZigbee, APP_ZIGBEE_ENDPOINT );
  assert( stZigbeeAppInfo.OnOffClient1 != NULL );
  if ( ZbZclClusterEndpointRegister( stZigbeeAppInfo.OnOffClient1 ) == false )
  {
    LOG_ERROR_APP( "Error during OnOff Client Endpoint Register for Endpoint 1." );
  }
  ...
  stRequest.profileId = APP_ZIGBEE_PROFILE_ID;
  stRequest.deviceId = APP_ZIGBEE_DEVICE_ID;
  stRequest.endpoint = APP_ZIGBEE_ENDPOINT2;
  ZbZclAddEndpoint( stZigbeeAppInfo.pstZigbee, &stRequest, &stConfig );
  assert( stConfig.status == ZB_STATUS_SUCCESS );

  /* Add OnOff Client Cluster for Endpoint 2 */
  stZigbeeAppInfo.OnOffClient2 = ZbZclOnOffClientAlloc( stZigbeeAppInfo.pstZigbee, APP_ZIGBEE_ENDPOINT2 );
  assert( stZigbeeAppInfo.OnOffClient2 != NULL );
  if ( ZbZclClusterEndpointRegister( stZigbeeAppInfo.OnOffClient2 ) == false )
  {
    LOG_ERROR_APP( "Error during OnOff Client Endpoint Register for Endpoint 2." );
  }
...
void APP_ZIGBEE_PrintApplicationInfo(void)
{
  ...
  LOG_INFO_APP( "  %s on Endpoint %d.", APP_ZIGBEE_CLUSTER_NAME, APP_ZIGBEE_ENDPOINT2 );
...
void APP_BSP_Button1Action(void)
{
  ...
  {
    /* Prepare destination - send to coordinator (address 0x0000) */
    memset( &stDest, 0, sizeof( stDest) );
    stDest.endpoint = APP_ZIGBEE_ENDPOINT;  // OnOff server endpoint (16)
    stDest.mode = ZB_APSDE_ADDRMODE_SHORT;
    stDest.nwkAddr = 0x0000;  // Coordinator address

    LOG_INFO_APP( "[ONOFF] SW1 pushed, sending 'TOGGLE' to coordinator endpoint %d", APP_ZIGBEE_ENDPOINT );
    eStatus = ZbZclOnOffClientToggleReq( stZigbeeAppInfo.OnOffClient1, &stDest, NULL, NULL );
...
void APP_BSP_Button2Action(void)
{
  ...
  if ( APP_ZIGBEE_IsAppliJoinNetwork() != false )
  {
    /* Prepare destination - send to coordinator (address 0x0000) */
    memset( &stDest, 0, sizeof( stDest) );
    stDest.endpoint = APP_ZIGBEE_ENDPOINT2;  // OnOff server endpoint (15)
    stDest.mode = ZB_APSDE_ADDRMODE_SHORT;
    stDest.nwkAddr = 0x0000;  // Coordinator address

    LOG_INFO_APP( "[ONOFF] SW2 pushed, sending 'TOGGLE' to coordinator endpoint %d", APP_ZIGBEE_ENDPOINT2 );
    eStatus = ZbZclOnOffClientToggleReq( stZigbeeAppInfo.OnOffClient2, &stDest, NULL, NULL );
    if ( eStatus != ZCL_STATUS_SUCCESS )
    {
      LOG_ERROR_APP( "[ONOFF] Error, OnOff Client Request failed (0x%02X).", eStatus );
    }
