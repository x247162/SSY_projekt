...
int main(void)
{
  ...
  if (DEV_Module_Init() == 0) {
    EPD_1in9_init();
    EPD_Display_Clear();
    EPD_Display_Startup_Sequence();
  }
  /* Display will be updated from Zigbee sensor callbacks every 15 seconds */
...
void MX_GPIO_Init(void)
{
  ...
  /* Configure GPIO pins for e-Paper display */
  /* PA10 = EPD RST (Reset output) */
  ...
  /* PA9 = EPD BUSY (Busy input) */
  GPIO_InitStruct.Pin = GPIO_PIN_9;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  /* Initialize RST pin to high */
